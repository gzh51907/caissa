                ...
                <div class="part_info mar_bot10" style="margin-bottom:50px">
                    <ul>
                        ...
                        <li>
                            ...
                            <p><em>{{date[datelength-1]}}</em></p>
                        </li>
                        <li>
                            <span class="tdgm">团队规模</span>
                            <p><em>团队总人数{{datelength}}</em></p>
                        </li>
                        <li>
                            <span class="cprq">产品人气</span>
                            <p><i class="num">{{data.id}}</i><em>人想去</em></p>
                        </li>
                    </ul>
                </div>
            </div>
...
export default {
    ...
    methods:{
        ...
        if(username){//有登陆
           this.$router.push({path:'/orderinf',query:{id:this.data.id,godate:this.godate}})
        }else{//没登录
        ...
        wantgo(){
             let username = localStorage.getItem('Authorization');
             if(username){//有登陆
                this.$refs.wg.innerHTML = this.$refs.wg.innerHTML=="我想去"?"想去":"我想去"
             }else{
               this.$router.push('/log');
             }
        }
    },
    updated(){//进入页面时默认选择第一个日期
