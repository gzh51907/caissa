...
export default {
    data(){
        return {
            menus:{
                "目的地":['伦敦','爱尔兰','爱丁堡','剑桥','曼彻斯特','哈利波特','英国'],
...
　       var scrollTop = window.pageYOffset||document.documentElement.scrollTop||document.body.scrollTop;
         let bd = this.$refs.bd
          if(scrollTop+document.documentElement.clientHeight>=bd.offsetHeight){
            setTimeout(async function() {//再次请求数据
      ...
      async request(item){//根据条件请求数据,query是个对象，里面是条件
        
        if(item=='价格从低到高'){
            this.count.sort((a,b)=>{return a.price-b.price})
        }else if(item=='价格从高到低'){
         this.count.sort((a,b)=>{return b.price-a.price})
        }else if(item=='默认综合排序'){
            this.count =[];//清空
          let {data} = await this.$axios.get('http://localhost:4399/searchlist/',{
            params:{num:0}
