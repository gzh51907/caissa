...
<el-form-item
  ...
  <el-form-item label="旅游性质">
    <el-checkbox-group v-model="sizeForm.tab">
      <el-checkbox-button label="观光" name="type"></el-checkbox-button>
      <el-checkbox-button label="亲子" name="type"></el-checkbox-button>
      <el-checkbox-button label="蜜月" name="type"></el-checkbox-button>
      <el-checkbox-button label="世界遗产" name="type"></el-checkbox-button>
      <el-checkbox-button label="毕业旅行" name="type"></el-checkbox-button>
    </el-checkbox-group>
  </el-form-item>
   <el-upload
   style="margin:10px"
 ...
 data() {
    ...
    methods: {
      async onSubmit() {
        ...
        if(this.canSumit==='success'&&this.canSumit2==='success'){//图片提交成功
          let result= await this.$instance.get('/tour/addtour',{
               params:{
                   sizeForm
               }
           })
           if(result.data.code==1){alert('旅游项目添加成功')}
        }else{
            alert('请将信息填完整')
        }
      },
